...
public class Main {
    ...
    public static int buildSegmentTree(int l, int r, int segInd, int[] arr) {
        if (l == r)
            return segArr[segInd] = arr[l];

        int mid = (l + r) / 2;

        return segArr[segInd] = Math.min(buildSegmentTree(l, mid, segInd * 2 + 1, arr),
    ...
    public static void update(int l, int r, int segInd, int ind, int val) {
        ...
        segArr[segInd] = Math.min(segArr[segInd * 2 + 1], segArr[segInd * 2 + 2]);
    }
